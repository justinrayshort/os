...
| --- | --- | --- | --- |
| `TOK-01` | Complete | The modern-adaptive skin consumes a unified shared token graph for typography, spacing, radius, component sizing, elevation, and motion. | `00-foundations.css` defines the shared `--sys-*` families, while `30-theme-modern-adaptive.css` remaps those tokens for the skin. |
...
| `TOK-03` | Complete | Interactive state colors (hover/active/focus/selection) use semantic tokens rather than repeated color literals. | Shared primitive selectors consume tokenized state colors and surfaces from `01-primitives.css`, and the modern-adaptive skin remaps those values in `30-theme-modern-adaptive.css`. |
| `TOK-04` | Complete | Layout spacing/radius values use shared spacing/radius/component tokens for new work. | Shared primitive selectors consume `--sys-space-*`, `--sys-radius-*`, and `--sys-comp-*` metrics rather than skin-specific layout literals. |
...
| `TOK-06` | Complete | Motion timings for shell transitions use shared motion tokens and support reduced-motion override suppression. | `--sys-motion-*` tokens are defined in `00-foundations.css`, and `04-accessibility-motion.css` suppresses transitions/animations for runtime or media-query reduced-motion scopes. |
...
| `CMP-02` | Complete | Shell chrome surfaces (windows, taskbar, menus, dialogs) share consistent visual rules for radius, borders, and elevation within the modern-adaptive scope. | Shared primitive selectors in `01-primitives.css` style shell/app surfaces through `data-ui-*` contracts, while `30-theme-modern-adaptive.css` remaps only the tokens that drive those visuals. |
| `CMP-03` | Complete | Component states (hover, active, focused, minimized, pressed, selected) are represented via semantic attributes and styled consistently. | Runtime and app markup now expose semantic `data-ui-*` state markers such as `data-ui-active`, `data-ui-selected`, `data-ui-pressed`, and `data-ui-minimized`. |
...
| `A11Y-03` | Complete | Interactive shell regions and menus use explicit roles and ARIA attributes appropriate to interaction patterns. | `role="toolbar"`, `group`, `menu`, `menuitem*`, `dialog`, `tablist`, `tab`, `tabpanel`, `listbox`, `option`, `aria-pressed`, `aria-haspopup`, `aria-expanded`, etc. Terminal transcript remains `role="log"` with `aria-live="polite"` and completion choices remain exposed through listbox/button semantics. |
| `A11Y-04` | Complete | Focus indicators are visible and theme-consistent for keyboard navigation using `:focus-visible`. | Shared primitive styling defines focus outlines via `--sys-focus-*`; keyboard-selected taskbar state remains visually distinct under modern-adaptive token remaps. |
| `A11Y-05` | Partial | Reduced-motion support includes both system preference fallback and runtime override. | `04-accessibility-motion.css` supports both media-query and runtime `data-reduced-motion` suppression. Manual verification matrix not yet codified before this change. |
...
| `INT-03` | Complete | Backdrop and blur effects are capability-gated and degrade gracefully. | Primitive styling limits those effects to supported surfaces and guards them with capability checks in `01-primitives.css`. |
...
| ID | Status | Acceptance criteria (objective) | Current evidence / gap |
| --- | --- | --- | --- |
| `RSP-01` | Complete | Shell has explicit responsive breakpoints for taskbar density, icon layout, menu sizing, and window sizing on narrow viewports. | `03-responsive.css` defines breakpoint behavior for icon grids, taskbar density, menu sizing, and window layout. |
| `RSP-02` | Complete | Small-screen window behavior prevents off-screen unusable windows (viewport clamping/resizing rules). | `03-responsive.css` constrains shell layout and small-screen window presentation for narrow viewports. |
...
| `DOC-06` | Complete | CI/local tooling includes machine-checkable UI conformance gates beyond documentation validation. | `cargo xtask docs ui-conformance` and `cargo xtask docs ui-inventory` now enforce token-only skin scopes, shared primitive adoption, and static icon-standardization regressions in shell component files. |
