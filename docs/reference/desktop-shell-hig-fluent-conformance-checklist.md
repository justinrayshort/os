...
| --- | --- | --- | --- |
| `TOK-01` | Complete | Fluent-modern theme defines named token layers for typography, spacing, radius, icon sizes, and motion durations in dedicated token files. | `crates/site/src/theme_shell/30-theme-fluent-modern-tokens-core.css` and `crates/site/src/theme_shell/32-theme-fluent-modern-theme-tokens.css` define `--fluent-font-*`, `--fluent-space-*`, `--fluent-radius-*`, `--fluent-icon-*`, `--fluent-motion-*`, shell semantic tokens, and terminal-specific `--terminal-*` tokens for transcript/composer hierarchy. |
...
| `INT-05` | Partial | HIG-aligned pointer target sizing and coarse-pointer/touch ergonomics are explicitly validated (target size thresholds + test matrix). | Coarse-pointer ergonomics are now tokenized and implemented across app surfaces and taskbar controls (`--fluent-app-touch-*` plus coarse-pointer responsive rules in `03-responsive-base.css` and Fluent overrides), and runtime taskbar/window heuristics now adapt layout pressure via viewport-aware planning. The terminal redesign preserves narrow-view prompt/completion spacing under the same responsive token model, but a documented target-size test matrix and evidence pack remain outstanding. |
